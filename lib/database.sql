...
-- Do this in Supabase Dashboard → Authentication → Sign In / Up → Third-party Auth
-- Add Provider: Clerk. Enter your Clerk domain (e.g. your-app.clerk.accounts.dev)
...
CREATE TABLE IF NOT EXISTS user_skills (
  ...
  skill_id              TEXT NOT NULL,           -- references competency_skills.id or domain string
...
CREATE TABLE IF NOT EXISTS scenarios (
  id                    TEXT PRIMARY KEY,        -- e.g. 'de-escalation', 'reinforcement'
  title                 TEXT NOT NULL,
  description           TEXT,
...
CREATE TABLE IF NOT EXISTS module_progress (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id               TEXT NOT NULL,
  module                TEXT NOT NULL CHECK (module IN ('simulator','rehearsal','reflective','adaptive')),
...
-- Scenarios and competency_skills are public (read-only for all authenticated users)
ALTER TABLE scenarios          ENABLE ROW LEVEL SECURITY;
ALTER TABLE competency_skills  ENABLE ROW LEVEL SECURITY;
ALTER TABLE achievements       ENABLE ROW LEVEL SECURITY;

-- Helper: extract Clerk user ID from JWT
-- auth.jwt() ->> 'sub' is the Clerk user ID when using the Clerk third-party integration
...
-- Assessments: trainees see their own; supervisors see their trainees
...
CREATE OR REPLACE VIEW rbt_dashboard_summary AS
SELECT
  up.user_id,
  up.level,
  up.total_xp,
  ...
  -- 40-Hour tracker
  ROUND(COALESCE(p.total_training_minutes, 0) / 60.0, 1)                   AS hours_trained,
  ROUND(LEAST(COALESCE(p.total_training_minutes, 0) / 60.0 / NULLIF(p.target_training_hours, 0) * 100, 100), 1) AS pct_of_target,
...
LEFT JOIN LATERAL (
  SELECT
    COUNT(*) FILTER (WHERE status = 'completed') AS modules_complete,
    COUNT(*)                                      AS modules_total,
    ROUND(COUNT(*) FILTER (WHERE status = 'completed') * 100.0 / NULLIF(COUNT(*), 0), 1) AS completion_pct
  FROM module_progress mp WHERE mp.user_id = up.user_id
) mc ON true
LEFT JOIN LATERAL (
  SELECT
    COUNT(DISTINCT date) FILTER (WHERE date >= CURRENT_DATE - 7) AS sessions_last_7d,
    ROUND(AVG(time_spent_minutes / NULLIF(sessions_started, 0)), 1) AS avg_session_mins
  FROM user_engagement ue WHERE ue.user_id = up.user_id
) eng ON true
LEFT JOIN LATERAL (
  SELECT ARRAY_AGG(DISTINCT skill_id) FILTER (WHERE is_weak_area = true) AS weak_domains
  FROM user_skills us WHERE us.user_id = up.user_id
) wa ON true
LEFT JOIN LATERAL (
  SELECT COUNT(*) FILTER (WHERE status = 'assigned') AS open_remediations
  FROM remediation_assignments ra WHERE ra.trainee_id = up.user_id
) ra ON true;

CREATE OR REPLACE VIEW competency_gap_report AS
SELECT
  user_id,
  skill_id                                AS domain,
  ...
  COUNT(*) FILTER (WHERE is_weak_area)   AS weak_count,
  MAX(last_practiced)                    AS last_practiced
FROM user_skills
GROUP BY user_id, skill_id
ORDER BY avg_gap DESC;

CREATE OR REPLACE VIEW drop_off_analysis AS
SELECT
  scenario_id,
  drop_off_point,
  COUNT(*)                               AS abandonment_count,
  ROUND(COUNT(*) * 100.0 / NULLIF(SUM(COUNT(*)) OVER (PARTITION BY scenario_id), 0), 1) AS pct_of_starts,
  MAX(started_at)                        AS most_recent
FROM scenario_sessions
WHERE status = 'abandoned' AND drop_off_point IS NOT NULL
...
-- ─── STEP 6: Seed scenarios ──────────────────────────────────────────────────

INSERT INTO scenarios (id, title, description, module, category, difficulty, xp_reward, skill_ids)
VALUES
  ('de-escalation',   'De-escalation Techniques',      'Practice calming a frustrated client during a transition activity.',              'simulator', 'behavior_intervention', 3, 150, ARRAY['de-escalation','behavior_reduction']),
  ('reinforcement',   'Positive Reinforcement',        'Implement reinforcement strategies effectively with a client on task completion.', 'simulator', 'behavior_intervention', 2, 120, ARRAY['reinforcement']),
  ('data-collection', 'Data Collection & ABC Notation','Accurately observe, record, and categorize behaviors in a live session.',         'simulator', 'data_collection',       2, 130, ARRAY['data-collection']),
  ('communication',   'Guardian Communication',        'Communicate professionally with a concerned guardian about their child.',          'simulator', 'professional_conduct',  2, 110, ARRAY['communication']),
  ('safety',          'Safety & Crisis Management',    'Apply safety protocols when a situation develops that requires crisis response.',  'simulator', 'behavior_intervention', 4, 180, ARRAY['safety','crisis-management'])
ON CONFLICT (id) DO NOTHING;

-- Seed competency skills
